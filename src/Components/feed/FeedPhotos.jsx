...
import { PHOTOS_GET, USER_GET } from "../../api"
...
const FeedPhotos = ({ page, total, user, setInfinite, setModal }) => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      ...
      const body = USER_GET(token)
      const res = await fetch(body.url, body.options).then((j) => j.json())

      if (user && res.id !== user && json.length > 0 && user !== json[0].author)
  ...
  if (!validateUser)
    return (
      <p className="text-sm text-[#f31] mt-8 mx-auto max-w-[50rem]">
