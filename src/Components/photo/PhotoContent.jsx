...
const PhotoContent = ({data, single}) => {
  ...
  return (
    <div className={`photo-content ${single ? 'grid-cols-1 h-auto static' : ''}`} >
    	<div className={`smmd:[grid-row:1/4] grid-rows-1 ${single ? '[grid-row:1!important] rounded overflow-hidden' : ''}`} >
    	...
    	<div className={`${single ? 'p-0 pt-4' : 'p-8 pb-0'}` } >
          ...
          {user.data && user.data.username === photo.author ? (
            <PhotoDelete id={photo.id} />
    				...
    				<Link className='hover:underline' to={`/perfil/${photo.author}`}>@{photo.author}</Link>
          )}
  				<span className='flex items-center'>
  					<Visualization width='16px' height='10px' className='inline-block mr-2' />
  					{photo.acessos}
  				</span>
  			</div>
  			<h1 className='title'>
