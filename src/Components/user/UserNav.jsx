...
const Navegar = ({ to, svg, children, isMobile, ...props }) => {
  return (
    <NavLink
      className={`${isMobile ? "user-btns-mobile" : "user-btns"}`}
      to={to}
      {...props}
    >
      {svg}
...
const UserNav = () => {
  ...
  return (
    <>
      {mobile && (
        <button
          className={`user-btn-mobile ${
            mobileMenu && "user-btn-mobile-active"
          }`}
          aria-label="Menu"
      ...
      <nav
        className={`user-nav ${
          mobileMenu && "duration-300 pointer-events-auto opacity-100 z-50"
        }`}
      >
        <Navegar
          to="/conta"
          svg={<MinhasFotos className="mr-2 sm:mr-0" />}
          isMobile={mobile}
          end
        >
          Minhas Fotos
        ...
        <Navegar
          ...
          svg={<Estatisticas className="mr-2 sm:mr-0" />}
          isMobile={mobile}
        >
          Estatisticas
        </Navegar>
        <Navegar
          to="/conta/postar"
          svg={<Adicionar className="mr-2 sm:mr-0" />}
          isMobile={mobile}
        >
          Adicionar Foto
        </Navegar>
        <button
          className={`${mobile ? "user-btns-mobile border-0" : "user-btns"}`}
          onClick={handleClick}
        >
